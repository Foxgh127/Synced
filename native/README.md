...
频道聊天和画面弹幕。电影主链路使用腾讯云 LiveKit SFU：放映端只上传一份，SFU
负责分发；SFU 故障时自动回退 WebRTC P2P，严格 NAT 下再使用腾讯云 TURN。
信令服务处理频道状态并签发短期 ICE/SFU 凭据，不保存媒体内容或聊天历史。
...
- Emby 媒体由本机 FFmpeg 尽量只做解复用/重封装，生成约 750 毫秒的 fragmented MP4；
  片段按 48 KiB 拆包，经每位观众各自的可靠乱序 DataChannel 发送，包含会话/版本/
  传输代次/时间轴/块号/CRC32；观看端先重组并等待 init，再按序送入 MSE。放映端
  保留最近 60 秒缓存，每条观众通道独立背压、限流补片和旧代次隔离，
  观众以 8 秒起播、24 秒目标、32 秒上限的 MSE 前向缓存抵抗 Wi-Fi 抖动。正常
  情况下这些分片经 LiveKit DataTrack 分发，SFU 故障时才恢复逐观看者 P2P
  DataChannel；质量仍受用户选择和端点实测能力约束，不会用原盘绕过预算。
...
- 信令短暂重连不会再销毁仍然健康的 SFU/P2P 观看链路；接收端按连续丢包、抖动和冻结情况
  以及真实解码耗时/丢帧压力逐级降低分辨率，网络和解码持续稳定后逐级恢复到用户
  设置的最高画质。电影接收端使用独立的约 180 ms 抖动目标，语音直连约 90 ms、
  中继约 125 ms；
...
- EXE 只使用受 TLS 保护的公网信令，不包含或启动本地信令服务器。腾讯云是主信令，
  阿里云是唯一备用信令；自动化测试使用独立的随机回环端口，不进入生产包。
...
Windows 包内含固定 SHA-256 的 FFmpeg 8.1 LGPL 独立可执行文件，仅用于 Emby
解复用/重封装与必要的单份音频转换。`npm run check` 会执行
...
SFU、coturn 与 Nginx。影片默认经 SFU 分发；SFU 故障时才切换 P2P，需要中继时
使用腾讯云 TURN。阿里云 `47.98.173.139` 只保留备用信令、Nginx 和证书续签，
不运行 STUN、TURN、LiveKit 或任何媒体中继：

1. 腾讯云开放 TCP `80/443/3478/7881`、UDP `3478/7882` 与 coturn relay
   端口范围；阿里云仅开放 TCP `80/443`。
