...
- Emby 媒体由本机 FFmpeg 生成共享时间轴、约 2 秒 GOP 对齐的 CMAF 分片。默认只启动
  一个共享 720p 辅助档；资源预算允许且观看端确有需求时，最多再启动一个
  original/high/low 档；空闲档位会真正停止，
...
- Android APK：输入频道码、扫描邀请二维码或邀请链接加入，支持硬件解码播放、
  ...
  调亮度，右侧上下滑动只调“同频”自身的 `<video>/<audio>` 音量，不修改系统媒体音量。
...
- 最多 8 人；所有人都可加入 48 kHz 全频 Opus 连麦。配置 LiveKit 时使用独立、
  microphone-only 的语音 SFU；SFU 不可用时只允许最多 3 人 Mesh，4–8 人不会退回
  `O(n²)` 连接。降噪按强度依次为“自然降噪”
  （平台保真处理）、“清晰人声”（语音隔离、人声频段增强与中等动态）和“强力消噪”
  （本地 DeepFilterNet3 AudioWorklet，断网也可加载）。三档使用不同的高通、增益
  与压缩参数，并共用 -3 dB 防爆音限幅。普通连麦目标为 48–64 kbps；
  共享伴奏时使用 72–96 kbps、带内 FEC 和可变码率，自适应边界为 32–128 kbps。即使
...
- 窗口声音要求 Windows 10 2004（内部版本 19041）或更高版本。Windows API
  ...
  helper 的 MMCSS 回调只进入丢旧优先的有界队列，独立 writer 写 stdout；Electron
  使用带 ACK/水位的 MessagePort，Renderer 再把 transferable block 送入 AudioWorklet
  ring。任一层中断都会进入自动重建；已有观看连接优先直接替换音轨，
...
3. EXE 与 APK 默认连接 `wss://synced.com.cn/signal`。运维手动切换或验证时可使用
   ...
   明文网络流量，自建 RFC1918/LAN 信令同样必须配置有效 TLS 并使用 `wss://`；
   代理或运营商也不能篡改信令内容。
